...
class HealthcareDataCleaner:
    ...
    def xml_to_dict(self, element):
        """Convert XML to dictionary, properly handling lists"""
        result = {}
        ...
        for child in element:
            ...
            if len(child) == 0:
                ...
                if child_counts[child_tag] > 1:
                    # This tag appears multiple times -> it's a list
                    if child_tag not in result:
                        result[child_tag] = []
                    result[child_tag].append(child.text if child.text else "")
                else:
                    # Single occurrence
                    result[child_tag] = child.text if child.text else ""
            else:
    ...
    def _create_comprehensive_patient_record(self, patient):
        ...
        try:
            ...
            # Extract all possible data recursively
            self._extract_patient_data(patient, record, 'patient')
            
            # Add hospital info if available
            if hasattr(self, 'data') and self.data:
                hospital_data = self.data
                if 'hospitalId' in hospital_data:
                    record['hospital_id'] = hospital_data.get('hospitalId')
                if 'hospitalName' in hospital_data:
                    record['hospital_name'] = hospital_data.get('hospitalName')
                if 'facilityType' in hospital_data:
                    record['facility_type'] = hospital_data.get('facilityType')
    ...
    def _extract_patient_data(self, data, record, prefix):
        """Recursively extract all patient data"""
        if isinstance(data, dict):
            for key, value in data.items():
                # Skip attributes and empty values
                ...
                if isinstance(value, dict):
                    ...
                    # Handle lists - extract first meaningful items
                    if value:
                        # For medical conditions, medications, etc., take the first one
                        if any(medical_term in key.lower() for medical_term in 
                              ['condition', 'medication', 'allergy', 'diagnosis', 'treatment', 'symptom']):
                            if isinstance(value[0], dict):
                                self._extract_patient_data(value[0], record, f"{full_key}_primary")
                            else:
                                record[f"{full_key}_primary"] = value[0]
            ...
            # Handle top-level lists (shouldn't happen with patient data)
