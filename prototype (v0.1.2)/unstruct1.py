...
class HealthcareDataCleaner:
    ...
    def xml_to_dict(self, element):
        """Enhanced XML to dictionary conversion for clinical data"""
        result = {}
        ...
        # Count children by tag name to detect lists
        child_counts = {}
        ...
        for child in element:
            ...
            if len(child) == 0:
                ...
                if child_counts[child_tag] > 1:
                    # Multiple occurrences -> store as list
                    if child_tag not in result:
                        result[child_tag] = []
                    result[child_tag].append(child_text)
                else:
                    # Single occurrence
                ...
                # Handle clinical data structures specifically
                if child_tag in ['encounter', 'labTest', 'relative', 'medication', 'patient']:
                    # These are always arrays in clinical data
                    if child_tag not in result:
                        result[child_tag] = []
                    result[child_tag].append(child_dict)
                elif child_counts[child_tag] > 1:
                    # Multiple occurrences -> store as list
                    if child_tag not in result:
                        result[child_tag] = []
                    result[child_tag].append(child_dict)
    ...
    def find_patients_data(self, data):
        ...
        def extract_all_patients(obj, path=""):
            if isinstance(obj, dict):
                # Check if this is a patient record (has patientId and demographics/medical info)
                if self._is_patient_record(obj):
                    patients.append(obj)
                
                # Check for nested patient structures
                for key, value in obj.items():
                    key_lower = str(key).lower()
                    
                    # Handle patient lists directly
                    if key_lower in ['patients', 'patient', 'patientrecords', 'patient_records']:
                        if isinstance(value, list):
                            ...
                            for item in value:
                                if self._is_patient_record(item):
                                    ...
                                    extract_all_patients(item, f"{path}.{key}")
                        elif isinstance(value, dict):
                            # This could be a single patient or a container
                            if self._is_patient_record(value):
                                patients.append(value)
                            else:
                                # It's a container, search deeper
                                extract_all_patients(value, f"{path}.{key}")
                        continue
                    
                    # Recursive search in other structures
                    extract_all_patients(value, f"{path}.{key}" if path else key)
                ...
                for i, item in enumerate(obj):
                    if self._is_patient_record(item):
                        ...
                        extract_all_patients(item, f"{path}[{i}]")
        
        extract_all_patients(data)
        
        # Remove duplicates while preserving order
        unique_patients = []
        seen_ids = set()
        for patient in patients:
            patient_id = patient.get('patientId') or patient.get('id') or patient.get('patientID')
            if patient_id and patient_id not in seen_ids:
                seen_ids.add(patient_id)
                unique_patients.append(patient)
            elif not patient_id:
                # If no ID, use the entire structure as identifier
                patient_str = json.dumps(patient, sort_keys=True)
                if patient_str not in seen_ids:
                    seen_ids.add(patient_str)
                    unique_patients.append(patient)
        
        # Use ASCII-friendly logging
        print(f"Found {len(unique_patients)} unique patient records")
        for i, patient in enumerate(unique_patients):
            patient_id = patient.get('patientId') or patient.get('id') or f'No-ID-{i+1}'
            print(f"  Patient {i+1}: {patient_id}")
        
        return unique_patients if unique_patients else None
    ...
    def _is_patient_record(self, obj):
        ...
        # Must have patient identifier or be in a patient-like structure
        has_patient_id = any(key.lower() in ['patientid', 'patient_id', 'id', 'patientid'] for key in obj.keys())
        ...
        demographic_indicators = sum(1 for key in obj.keys() if any(term in key.lower() for term in [
            ...
            'firstname', 'lastname', 'birth', 'address', 'phone', 'email',
            'diagnosis', 'treatment', 'symptom', 'allergy', 'prescription'
        ]))
        
        # Consider it a patient record if it has patient ID OR sufficient demographic indicators
        return has_patient_id or demographic_indicators >= 2

    def create_analysis_dataset(self):
        """Create clean, structured analysis dataset with ALL patients"""
        try:
            patients_data = self.find_patients_data(self.data)
            ...
            for i, patient in enumerate(patients_data):
                print(f"  Processing patient {i+1}: {patient.get('patientId', 'Unknown')}")
                record = self._create_comprehensive_patient_record(patient)
                if record:
            ...
            if structured_data:
                ...
                print("No structured data created from patients")
                return self._create_fallback_dataset()
    ...
    def _create_comprehensive_patient_record(self, patient):
        """Create a comprehensive patient record with all available data"""
        record = {}
        
        try:
            # Basic patient info
            record['patient_id'] = patient.get('patientId') or patient.get('id') or f'Unknown_{hash(str(patient))}'
            
            # Extract hospital metadata from root level
            if hasattr(self, 'data') and self.data:
                if 'hospitalId' in self.data:
                    record['hospital_id'] = self.data['hospitalId']
                if 'dataTimestamp' in self.data:
                    record['data_timestamp'] = self.data['dataTimestamp']
                if 'facilityType' in self.data:
                    record['facility_type'] = self.data['facilityType']
            
            # Extract all patient data
            self._extract_patient_data(patient, record, 'patient')
            
            # Enhanced encounter data extraction
            self._extract_encounter_data(patient, record)
            
            # Enhanced clinical data extraction for XML
            self._extract_clinical_data(patient, record)
    ...
    def _extract_clinical_data(self, patient, record):
        """Specialized extraction for clinical data structures in XML"""
        try:
            # Extract encounters with all clinical details
            if 'encounters' in patient:
                encounters_data = patient['encounters']
                # Handle both list and single encounter formats
                if isinstance(encounters_data, list):
                    encounters = encounters_data
                elif 'encounter' in encounters_data:
                    encounters = encounters_data['encounter']
                    if not isinstance(encounters, list):
                        encounters = [encounters]
                else:
                    encounters = []
                
                record['encounters_count'] = len(encounters)
                
                for i, encounter in enumerate(encounters[:3]):  # First 3 encounters
                    prefix = f"encounter_{i}"
                    
                    # Basic encounter info
                    for field in ['encounterId', 'date', 'type', 'provider', 'department']:
                        if field in encounter:
                            record[f"{prefix}_{field}"] = encounter[field]
                    
                    # Vitals
                    if 'vitals' in encounter:
                        vitals = encounter['vitals']
                        for vital_field in ['bpSystolic', 'bpDiastolic', 'heartRate', 'temperature', 'height', 'weight']:
                            if vital_field in vitals:
                                record[f"{prefix}_{vital_field}"] = vitals[vital_field]
                    
                    # Lab Results
                    if 'labResults' in encounter:
                        lab_results = encounter['labResults']
                        if 'labTest' in lab_results:
                            lab_tests = lab_results['labTest']
                            if not isinstance(lab_tests, list):
                                lab_tests = [lab_tests]
                            
                            record[f"{prefix}_labTests_count"] = len(lab_tests)
                            for j, test in enumerate(lab_tests[:5]):  # First 5 lab tests
                                for field in ['testName', 'value', 'unit', 'referenceRange']:
                                    if field in test:
                                        record[f"{prefix}_lab_{j}_{field}"] = test[field]
                    ...
                    if 'diagnoses' in encounter:
                        ...
                        if not isinstance(diagnoses, list):
                            diagnoses = [diagnoses]
                        record[f"{prefix}_diagnoses_count"] = len(diagnoses)
                        for j, diagnosis in enumerate(diagnoses[:3]):
                            record[f"{prefix}_diagnosis_{j}"] = diagnosis
                    
                    # Clinical Notes
                    if 'clinicalNotes' in encounter:
                        record[f"{prefix}_clinicalNotes"] = encounter['clinicalNotes']
            
            # Extract family history
            if 'familyHistory' in patient:
                family_history = patient['familyHistory']
                if 'relative' in family_history:
                    relatives = family_history['relative']
                    if not isinstance(relatives, list):
                        relatives = [relatives]
                    
                    record['familyHistory_relatives_count'] = len(relatives)
                    for i, relative in enumerate(relatives[:5]):  # First 5 relatives
                        for field in ['relationship', 'condition', 'ageAtDiagnosis']:
                            if field in relative:
                                record[f"familyHistory_relative_{i}_{field}"] = relative[field]
            
            # Extract medications
            if 'medications' in patient:
                medications_data = patient['medications']
                if 'medication' in medications_data:
                    meds = medications_data['medication']
                    if not isinstance(meds, list):
                        meds = [meds]
                    
                    record['medications_count'] = len(meds)
                    for i, med in enumerate(meds[:5]):  # First 5 medications
                        for field in ['name', 'dosage', 'frequency', 'route', 'startDate']:
                            if field in med:
                                record[f"medication_{i}_{field}"] = med[field]
        
        except Exception as e:
            print(f"Error extracting clinical data: {e}")

    def _extract_patient_data(self, data, record, prefix):
        """Enhanced recursive data extraction with comprehensive array handling"""
        if isinstance(data, dict):
            for key, value in data.items():
                ...
                clean_key = full_key.replace('patient_', '').replace('demographics_', '')\
                                  .replace('personalInfo_', '').replace('medicalHistory_', '')\
                                  .replace('vitalSigns_', '').replace('contactInfo_', '')
                
                if isinstance(value, dict):
                    # Recursively extract nested dictionaries
                    self._extract_patient_data(value, record, full_key)
                    
                elif isinstance(value, list):
                    # ENHANCED: Extract more array items
                    record[f"{clean_key}_count"] = len(value)
                    
                    # Extract first 5 items for important medical arrays
                    if any(medical_term in key.lower() for medical_term in 
                          ['allergies', 'medications', 'conditions', 'labresults', 'encounters', 'chronicconditions']):
                        for i, item in enumerate(value[:5]):  # Increased from 2 to 5
                            if isinstance(item, dict):
                                # Extract key fields from medical objects
                                if 'name' in item:
                                    record[f"{clean_key}_{i}_name"] = item['name']
                                if 'diagnosed' in item:
                                    record[f"{clean_key}_{i}_diagnosed"] = item['diagnosed']
                                if 'dosage' in item:
                                    record[f"{clean_key}_{i}_dosage"] = item['dosage']
                                # Extract lab results
                                if 'results' in item and isinstance(item['results'], dict):
                                    for lab_key, lab_value in item['results'].items():
                                        record[f"{clean_key}_{i}_{lab_key}"] = lab_value
                                # Extract test information
                                if 'testName' in item:
                                    record[f"{clean_key}_{i}_testname"] = item['testName']
                                if 'status' in item:
                                    record[f"{clean_key}_{i}_status"] = item['status']
                            else:
                                record[f"{clean_key}_{i}"] = item
                    
                    # Extract simple values
                    simple_values = [item for item in value[:3] if isinstance(item, (str, int, float))]
                    for i, item in enumerate(simple_values):
                        record[f"{clean_key}_{i}"] = item
                        
                else:
                    # Simple value - include hospital metadata
            ...
            for i, item in enumerate(data[:3]):
                if isinstance(item, (str, int, float)):
                    record[f"{prefix}_{i}"] = item

    def _extract_encounter_data(self, patient, record):
        """Specialized extraction for encounter data"""
        if 'encounters' in patient and isinstance(patient['encounters'], list):
            for i, encounter in enumerate(patient['encounters'][:3]):  # First 3 encounters
                prefix = f"encounter_{i}"
                
                # Basic encounter info
                if 'encounterId' in encounter:
                    record[f"{prefix}_id"] = encounter['encounterId']
                if 'date' in encounter:
                    record[f"{prefix}_date"] = encounter['date']
                if 'type' in encounter:
                    record[f"{prefix}_type"] = encounter['type']
                if 'doctor' in encounter:
                    record[f"{prefix}_doctor"] = encounter['doctor']
                if 'diagnosis' in encounter:
                    record[f"{prefix}_diagnosis"] = encounter['diagnosis']
                if 'specialty' in encounter:
                    record[f"{prefix}_specialty"] = encounter['specialty']
                if 'notes' in encounter:
                    record[f"{prefix}_notes"] = encounter['notes']
                
                # Vitals data
                if 'vitals' in encounter and isinstance(encounter['vitals'], dict):
                    for vital_key, vital_value in encounter['vitals'].items():
                        record[f"{prefix}_vital_{vital_key}"] = vital_value
                
                # Medications Prescribed
                if 'medicationsPrescribed' in encounter:
                    meds = encounter['medicationsPrescribed']
                    record[f"{prefix}_meds_prescribed_count"] = len(meds)
                    for j, med in enumerate(meds[:3]):
                        if 'name' in med:
                            record[f"{prefix}_med_prescribed_{j}_name"] = med['name']
                        if 'dosage' in med:
                            record[f"{prefix}_med_prescribed_{j}_dosage"] = med['dosage']
                        if 'frequency' in med:
                            record[f"{prefix}_med_prescribed_{j}_frequency"] = med['frequency']
                
                # Medications Administered
                if 'medicationsAdministered' in encounter:
                    meds = encounter['medicationsAdministered']
                    record[f"{prefix}_meds_administered_count"] = len(meds)
                    for j, med in enumerate(meds[:3]):
                        if 'name' in med:
                            record[f"{prefix}_med_administered_{j}_name"] = med['name']
                        if 'dosage' in med:
                            record[f"{prefix}_med_administered_{j}_dosage"] = med['dosage']
                        if 'route' in med:
                            record[f"{prefix}_med_administered_{j}_route"] = med['route']
                
                # Lab Results
                if 'labResults' in encounter:
                    labs = encounter['labResults']
                    record[f"{prefix}_lab_results_count"] = len(labs)
                    for j, lab in enumerate(labs[:3]):
                        if 'testName' in lab:
                            record[f"{prefix}_lab_{j}_testname"] = lab['testName']
                        if 'status' in lab:
                            record[f"{prefix}_lab_{j}_status"] = lab['status']
                        if 'results' in lab and isinstance(lab['results'], dict):
                            for result_key, result_value in lab['results'].items():
                                record[f"{prefix}_lab_{j}_{result_key}"] = result_value
                        if 'units' in lab:
                            if isinstance(lab['units'], dict):
                                for unit_key, unit_value in lab['units'].items():
                                    record[f"{prefix}_lab_{j}_{unit_key}_unit"] = unit_value
                            else:
                                record[f"{prefix}_lab_{j}_units"] = lab['units']
                        if 'notes' in lab:
                            record[f"{prefix}_lab_{j}_notes"] = lab['notes']
    ...
    def _create_fallback_dataset(self):
        ...
        try:
            ...
            flat_data = self._flatten_entire_structure(self.data)
            if flat_data:
...
# The rest of the HealthcareCleanerGUI class remains exactly the same...
...
BEST FOR: Specialized domains, reproducible pipelines"""
    ...
    def _analyze_structure(self, data):
        ...
        patients = self.cleaner.find_patients_data(data)
    ...
    def apply_selected_operations(self):
        ...
        self.cleaner.analysis_dataset = self.cleaner.create_analysis_dataset()
        if self.cleaner.analysis_dataset is not None and not self.cleaner.analysis_dataset.empty:
            ...
            self.cleaner.cleaning_report.append("Could not create analysis dataset")
    ...
    def show_cleaning_results(self):
        ...
        if self.cleaner.analysis_dataset is not None and not self.cleaner.analysis_dataset.empty:
            ...
            patients_data = self.cleaner.find_patients_data(self.cleaner.data)
    ...
    def _export_fallback_csv(self):
        ...
        try:
            ...
            flat_data = self.cleaner._flatten_entire_structure(self.cleaner.data)
            if flat_data:
                df = pd.DataFrame([flat_data])
                filename = filedialog.asksaveasfilename(
                    defaultextension=".csv", 
